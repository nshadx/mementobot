...
internal class RouteBuilder(IServiceCollection services)
{
    ...
    public RouteBuilder Command<TRouteHandler>(string name) where TRouteHandler : class, IRouteHandler => When<TRouteHandler>(context => context.Update.Message is { Text: { } text } && text.Length >= name.Length && text[..name.Length].Equals(name, StringComparison.InvariantCultureIgnoreCase));

    public RouteBuilder File<TRouteHandler>(string pattern) where TRouteHandler : class, IRouteHandler => When<TRouteHandler>(context => context.Update.Message?.Document is { FileName: { } fileName } && Regex.IsMatch(fileName, pattern));

    public RouteBuilder Callback<TRouteHandler>(Func<string, bool> predicate) where TRouteHandler : class, IRouteHandler => When<TRouteHandler>(context => context.Update.CallbackQuery is { Data: { } data } && predicate(data));
